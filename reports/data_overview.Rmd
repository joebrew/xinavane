...
Though we have very little post-MDA data under observation, and we cannot account for seasonality (since there is less than a year's worth of data), we can still try to assess the effect of the MDA on worker absenteeism.  To this end, we use logistic regression to estimate the likelihood of absenteeism as a function of pre-/post- MDA, adjusting for worker type. 

```{r}
temp <- df %>%
  dplyr::select(worker_type, date, absence)
# Assign wheter pre or post MDA
temp$mda <- temp$date >= as.Date('2016-01-18')

# Relevel factors
temp$mda <- ifelse(temp$mda, 'after', 'before')
temp$mda <- factor(temp$mda, levels = c('before', 'after'))

temp$worker_type <- factor(temp$worker_type,
                           levels = c('general services',
                                      'factory',
                                      'agriculture'))

# Relevel the absence
temp$absence <- ifelse(temp$absence, 1, 0)

# Estimate model
fit <- glm(absence ~ mda + worker_type, 
          data = temp,
          family = 'binomial')
...
if('x.csv' %in% dir()){
  ...
  x <- data.frame(ors)
  x$characteristic <- row.names(x); row.names(x) <- NULL
  names(x)[2:3] <- c('lwr', 'upr')
  x <- x[,c('characteristic', 'OR', 'lwr', 'upr')]
  x$characteristic <- gsub('mda','MDA: ', x$characteristic)
  x$characteristic <- gsub('worker_type', 'Worker type: ', x$characteristic)
  write_csv(x, 'x.csv')
}
...
In the model, after adjustment for worker type, we find that the odds of absence is significantly lower following the MDA (odds ratio of 0.85, 95% confidence interval of 0.83 to 0.87). In other words, the MDA was associated with a reduction of 14.5% in worker absenteeism. 
